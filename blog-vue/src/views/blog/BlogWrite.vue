        ...
        <el-form :model="articleForm" ref="articleForm" :rules="rules">
          ...
          <el-form-item label="文章标签" prop="tagList">
            <el-checkbox-group v-model="articleForm.tagList">
              <el-checkbox v-for="t in tagList" :key="t.id" :label="t.id" name="tagList">{{t.tagName}}</el-checkbox>
            </el-checkbox-group>
          </el-form-item>
        </el-form>
    ...
    methods: {
      getArticleById(id) {
        ...
        getArticleById(id).then(data => {
          ...
          let tagList = this.articleForm.tagList.map(function (item) {
            return item.id;
          })

          this.articleForm.tagList = tagList
